...
public OnPlayerDisconnect(playerid,reason){
	if(GetPVarInt(playerid,"PlayerLogged")){
	    ...
	    new query[102-2-2-2+16+11+4];
	    mysql_format(mysql_connection,query,sizeof(query),"update`users`set`last_ip`='%e',`login_date`='%02i/%02i/%d %02i:%02i,`payday_time`='%i''where`id`='%i'",temp_ip,temp_day,temp_month,temp_year,temp_hour,temp_minute,player[playerid][payday_time],player[playerid][id]);
		...
		case dAuthorization:{
		    if(response){
				...
				if(cache_get_row_count(mysql_connection)){
					...
					player[playerid][payday_time]=cache_get_field_content_int(0,"payday_time",mysql_connection);
			            ...
			            if(cache_get_row_count(mysql_connection)){
			                ...
			                if(cache_get_row_count(mysql_connection)){
			                    ...
			                    for(new i=0; i!=cache_get_row_count(mysql_connection); i++){
			                        ...
			                        format(temp_string,sizeof(temp_string),""WHITE"%s [%d]%s %s",temp_name,temp_playerid,temp_connect,temp_bonus);
...
@__general_timer(){
	foreach(new i:Player){
	    if(GetPVarInt(i,"PlayerLogged")){
			player[i][payday_time]++;
			if(player[i][payday_time]>=PAYDAY_TIME){
				@__payday(i);
			}
	    }
	}
	    ...
	    if(player[playerid][experience]>=player[playerid][level]*NEEDED_EXPERIENCE){
	        ...
	        if(strlen(player[playerid][referal_name])>=MIN_PROMOCODE_LEN){
	            ...
	            if(player[playerid][level]==3 && !player[playerid][experience]){
					mysql_format(mysql_connection,query,sizeof(query),"select`money`from`users`where`name`='%e'",player[playerid][referal_name]);
					mysql_query(mysql_connection,query);
					if(cache_get_row_count(mysql_connection)){
						new temp_money;
						temp_money=cache_get_field_content_int(0,"money",mysql_connection);
						new temp_playerid;
						sscanf(player[playerid][name],"u",temp_playerid);
						if(GetPVarInt(temp_playerid,"PlayerLogged")){
						    new string[47-2+MAX_PLAYER_NAME];
						    format(string,sizeof(string),"Вы получили бонус за приглашённого игрока - %s",player[playerid][name]);
						    SendClientMessage(temp_playerid,C_GREEN,string);
						    GivePlayerMoney(temp_playerid,50000);
						    player[temp_playerid][money]+=50000;
						    mysql_format(mysql_connection,query,sizeof(query),"update`users`set`money`='%i'where`id`='%i'",player[temp_playerid][money],player[temp_playerid][id]);
						}
						else{
						    mysql_format(mysql_connection,query,sizeof(query),"update`users`set`money`='%i'where`name`='%e'",temp_money+50000,player[playerid][name]);
						}
						mysql_query(mysql_connection,query);
					}
				}
	            ...
	            if(cache_get_row_count(mysql_connection)){
	                ...
	                if(player[playerid][level]==temp_level && !player[playerid][experience]){
	                    GivePlayerMoney(playerid,temp_money);
						...
						new string[36-2+MAX_PROMOCODE_LEN];
						format(string,sizeof(string),"Вы получили бонус [%s%s ] по промокоду - %s",temp_money,temp_experience,temp_promocode);
						SendClientMessage(playerid,C_BLUE,string);
	                }
	            }
	        }
